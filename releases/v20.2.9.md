...
- Earlier `st_simplify` with `NaN` caused a node to crash. The behavior has been changed to align it more to PostGIS. [#63798][#63798]
...
- Fixed a rare bug which can cause a crash if [`CREATE SCHEMA`](../v20.2/create-schema.html) is run in a transaction which began prior to a cluster version update which enables that statement. [#63786][#63786]
- Fixed a panic which can occur in cases after a [`RESTORE`](../v20.2/restore.html) of a table using user-defined types. [#63550][#63550]
...
- Fixed a bug where upon failure of [`CREATE TABLE AS`](../v20.2/create-table-as.html) or `CREATE MATERIALIZED VIEW` tables would be left in an invalid non-public state until GC instead of being marked as dropped, possibly causing spurious validation failures. The bug was introduced in earlier v20.2 testing releases. [#63987][#63987]
- Fixed `crdb_internal.encode_key` for user-defined types. This would previously error. [#64031][#64031]
...
- A correctness bug, which caused partitioned index scans to omit rows where the value of the first index column was `NULL`, has been fixed. This bug was present since v19.2.0. [#64049][#64049]
- Previously, CockroachDB would either return an error or crash when comparing an infinite date coming from a subquery against a timestamp. [#64075][#64075]
- Fixed a bug where multiple concurrent invocations of [`cockroach debug zip`](../v20.2/cockroach-debug-zip.html) could yield cluster instability. This bug had been present since CockroachDB v20.1. [#64084][#64084]
...
- Reduce memory usage in some write-heavy workloads. Improve write performance when a lot of files are being deleted. Address one case where too many small sstable files were generated. [#64599][#64599]

### Build changes

- CockroachDB now builds on Ubuntu 20.10 and other distos using gcc-10. [#62201][#62201]
...
[80b08d16f]: https://github.com/cockroachdb/cockroach/commit/80b08d16f
